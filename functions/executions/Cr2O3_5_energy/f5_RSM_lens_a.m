...
samples = searchSamples_v2(...
    {{'Batch','Cr2O3_energy';'RSM','a';'Sub','a-Al2O3'}},true);
...
scatter(ax(1),X,Y(sortIdx,1),"k^",...
    MarkerFaceColor = "g",...
    MarkerFaceAlpha = .4,...
    DisplayName = "a_{out} (22.6)",...
    SizeData = fs*8)
plot(ax(2),sort(X),Y(sortIdx,2),"k--",HandleVisibility="off")
scatter(ax(2),X,Y(sortIdx,2),"ko",...
    MarkerFaceColor = "g",...
    MarkerFaceAlpha = .4,...
    DisplayName = "c_{in} (22.6)",...
    SizeData = fs*8)
plot(ax(1),sort(X),Y(sortIdx,3),"k--",HandleVisibility="off")
scatter(ax(1),X,Y(sortIdx,3),"k^",...
    MarkerFaceColor = "r",...
    MarkerFaceAlpha = .4,...
    DisplayName = "a_{out} (30.0)",...
    SizeData = fs*8)
plot(ax(1),sort(X),Y(sortIdx,4),"k--",HandleVisibility="off")
scatter(ax(1),X,Y(sortIdx,4),"k>",...
    MarkerFaceColor = "r",...
    MarkerFaceAlpha = .4,...
    DisplayName = "a_{in} (30.0)",...
    SizeData = fs*8)
...
exportgraphics(F,"../Plots/Cr2O3/5 energy/5-RSM_lens_a.pdf")
exportgraphics(F,"../Plots/Cr2O3/5 energy/5-RSM_lens_a.png","Resolution",250)