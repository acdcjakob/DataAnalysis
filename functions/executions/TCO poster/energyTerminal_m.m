...
Sy = scatter(ax,x,eyy*100,72,"<k","filled",...
    "Markerfacecolor",cs(1,:),...
    "displayname","\epsilon_{yy} (03.0)");
Px = plot(ax,x,eyy*100,"--",Color=cs(1,:),...
    HandleVisibility="off");

%%
[ax2,f2] = createAxes(1,450);
...
l2b = legend(ax2b,"location","southeast");
l2b.Title.String = "shear angle of film";
l2b.FontSize = 12;
l2b.Color="w";
title(ax2,"(30.0) Cr2O3 on Al2O3")

SaPar = scatter(ax2,x,abs([y.tiltAngle_cPar]'*60),72,"^","filled",...
    Markerfacecolor=cs(2,:),...
    DisplayName="|| to {\itc}",...
    MarkerFaceAlpha=.9);
PaPar = plot(ax2,x,abs([y.tiltAngle_cPar]'*60),...
    Color=cs(2,:),...
    Linestyle="--",...
    HandleVisibility="off");
...
SsPar = scatter(ax2b,x,abs([y.shearAngle_cPar]'*3600),72,"^","filled",...
    Markerfacecolor=cs(3,:),...
    DisplayName="|| to {\itc}",...
    MarkerFaceAlpha=.9);
PsPar = plot(ax2b,x,abs([y.shearAngle_cPar]'*3600),...
    Color=cs(3,:),...
    Linestyle="--",...
    HandleVisibility="off");
SsPerp = scatter(ax2b,x,abs([y.shearAngle_cPerp]'*3600),72,"o","filled",...
    Markerfacecolor=cs(3,:),...
    DisplayName="\perp to {\itc}",...
    MarkerFaceAlpha=.9);
